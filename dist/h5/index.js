...
class Compiler {
    constructor(appPath, entryFile, isUi) {
        ...
        this.entryFilePath = util_1.resolveScriptPath(path.join(this.sourcePath, entryFile || config_1.default.ENTRY));
    ...
    processEntry(code, filePath) {
        ...
        const routerBasename = isMultiRouterMode
            ...
            : helper_1.addLeadingSlash(helper_1.stripTrailingSlash(lodash_1.get(this.h5Config, 'router.basename')));
        ...
        const ClassDeclarationOrExpression = {
            enter(astPath) {
                ...
                if (!node.superClass)
                    return;
                if (helper_1.isTaroClass(astPath)) {
            ...
            ClassMethod: {
                ...
                `;
                        ...
                        const buildFuncBody = fp_1.pipe(...lodash_1.compact([
                            ...
                            constants_2.providerComponentName && storeName && wrapWithProvider,
                            wrapWithFuncBody
                        ]));
                ...
                if (keyName === 'pages' && t.isArrayExpression(value)) {
                    ...
                    if (subPackageParent) {
                        ...
                        value.elements.forEach((v) => {
                            ...
                            const pageName = helper_1.removeLeadingSlash(pagePath);
        ...
        babel_traverse_1.default(ast, {
                    ...
                    if (keyName === 'state') {
                        ...
                        if (isMultiRouterMode) {
                            lodash_1.merge(customRoutes, lodash_1.transform(pages, (res, [pageName, filePath], key) => {
                                res[helper_1.addLeadingSlash(pageName)] = helper_1.addLeadingSlash(filePath);
                            }, {}));
                        }
                    }
                }
                ...
                enter: (astPath) => {
                    ...
                    if (source.value === '@tarojs/taro') {
                        ...
                        if (specifier) {
                            ...
                            providerImportName = constants_2.providerComponentName;
                    ...
                    if (util_1.isAliasPath(source.value, pathAlias)) {
                        source.value = this.transformToTempDir(util_1.replaceAliasPath(filePath, source.value, pathAlias));
                    }
                    if (!util_1.isNpmPkg(source.value)) {
                        if (source.value.indexOf('.') === 0) {
                            ...
                            if (pathArr.indexOf('pages') >= 0) {
                            ...
                            else if (constants_1.REG_SCRIPTS.test(source.value) || path.extname(source.value) === '') {
                                /* 移除后缀名 */
                                ...
                                source.value = util_1.promoteRelativePath(path.relative(filePath, removeExtPath)).replace(/\\/g, '/');
                            }
                        }
                    }
                }
            ...
            Program: {
                exit(astPath) {
                    ...
                    const extraNodes = [
                        !hasNerv && astConvert_1.convertSourceStringToAstExpression(`import ${constants_2.nervJsImportDefaultName} from 'nervjs'`),
                        tabBar && astConvert_1.convertSourceStringToAstExpression(`import { View, ${constants_2.tabBarComponentName}, ${constants_2.tabBarContainerComponentName}, ${constants_2.tabBarPanelComponentName}} from '@tarojs/components'`),
              ...
              const _taroHistory = createHistory({
                ...
                basename: "${routerBasename}",
                customRoutes: ${JSON.stringify(customRoutes)},
                firstPagePath: "${helper_1.addLeadingSlash(firstPage ? firstPage[0] : '')}"
              });
            `),
            ...
            return this.pages.map(([pageName, filePath], k) => {
                const createFuncBody = () => {
                    const shouldLazyloadPage = typeof routerLazyload === 'function'
                        ? routerLazyload(pageName)
                        : routerLazyload;
                    const route = helper_1.createRoute({
                        pageName,
                        lazyload: shouldLazyloadPage,
                        isIndex: k === 0
                    });
                    ...
                    ClassMethod: {
                        exit(astPath) {
                            ...
                            if (isRender) {
                                const buildFuncBody = fp_1.pipe(...lodash_1.compact([
                                    createFuncBody,
                                    tabBar && fp_1.partial(wrapWithTabbar, [helper_1.addLeadingSlash(pageName)]),
                                    constants_2.providerComponentName && storeName && wrapWithProvider,
                                    wrapWithFuncBody
                                ]));
                                node.body = astConvert_1.convertSourceStringToAstExpression(buildFuncBody(pages), { preserveComments: true });
                            ...
                            node.body.forEach((bodyNode) => {
                                if (t.isExpressionStatement(bodyNode)
                                    ...
                                    if (t.isObjectExpression(mountApisOptNode)) {
                                        const valueNode = t.stringLiteral(helper_1.addLeadingSlash(pageName));
        ...
        const defaultVisitor = {
            ...
            ImportDeclaration: {
                enter: (astPath) => {
                    ...
                    if (!util_1.isNpmPkg(source.value)) {
                        if (constants_1.REG_SCRIPTS.test(source.value) || path.extname(source.value) === '') {
                            const absolutePath = path.resolve(filePath, '..', source.value);
                            const dirname = path.dirname(absolutePath);
                            const extname = path.extname(absolutePath);
                            const realFilePath = util_1.resolveScriptPath(path.join(dirname, path.basename(absolutePath, extname)));
...
function build(appPath, buildConfig) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield util_1.checkCliAndFrameworkVersion(appPath, "h5" /* H5 */);
